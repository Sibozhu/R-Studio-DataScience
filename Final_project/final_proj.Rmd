---
title: "MA415 Final Project- Tweets' Analysis of President Trump"
author: "Sibo Zhu"
date: "2017/12/15"
output: pdf_document
---
...
```{r}
clst <- removeSparseTerms(tdm,sparse = 0.965)     # removing sparse terms
print(dimnames(clst)$Terms)
clst2 <- as.matrix(clst)
clst3 <- t(clst2)   # transpose matrix to cluster documents
set.seed(122)   # setting seed
k <- 6   # k value
kmeansResult <- kmeans(clst3,k)
...
```{r}
distMatrix <- dist(scale(clst2))      # cluster terms
fit <- hclust(distMatrix, 
              method = "complete"
              )

p <- plot(fit, xlab="")     # show cluster dendrogram 
