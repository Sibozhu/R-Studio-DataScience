...
ageData <- read_excel("INCOME.xlsx", sheet = "data", range = "A123:J129",
                          col_names = FALSE, col_types = "text")
...
colnames(ageData_1)      <- c("RFG_AGE", "TTL_US", "LESS_20T",
                           "20_39T", "40_59T", "60_79T",
                           "80_99T", "100_119T","120_139T","140T_MORE")
...
ageData_2 <- within(ageData_1, rm(TTL_US))
...
ageData_2$RFG_AGE    <- as.factor(ageData_2$RFG_AGE)
...
ageData_2[, 2:9]    <- sapply(ageData_2[, 2:9], as.numeric)
...
ageData_3 <- as_tibble(melt(ageData_2, id = 1))
...
names(ageData_3)[2]    <- "Income Range"
names(ageData_3)[3]    <- "Number of Unit"
...
ageData_SUM <- aggregate(ageData_3$`Number of Unit`, 
                           by = list(category = ageData_3$`Income Range`),
                           FUN=sum, na.rm = TRUE)
...
names(ageData_SUM)[1] <- "Income Range"
names(ageData_SUM)[2] <- "Total Number of Unit"
...
ageDataWithTotal <- merge(x = ageData_3, y = ageData_SUM, by = "Income Range", all = TRUE)
names(ageDataWithTotal)[4] <- "Total Number of Unit by Income Range"
ageDataWithTotal$Percentage <- 100*(ageDataWithTotal$`Number of Unit` /ageDataWithTotal$`Total Number of Unit by Income Range`)
rm(list = ls(pattern = "^T_"))
...
PLOT_AGE_PERC <- ggplot(data = ageDataWithTotal, aes(x = `Income Range`, y = Percentage)) +
                 geom_col(aes(fill = `RFG_AGE`)) +
                 labs(title = "Refrigerator Age Percentage") +
                 coord_flip()

PLOT_AGE <- ggplot(data = ageDataWithTotal, aes(x = `Income Range`, y = `Number of Unit`)) +
             geom_col(aes(fill = `RFG_AGE`)) +
             theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
             labs(title = "Refrigerator Age Histogram by Income Range, 2015")
```
